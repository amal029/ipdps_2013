...
% respect data dependencies between tasks. And finally, data
...
% dividing the tasks evenly across the available processors. However,
...
% % map the task graph to a heterogeneous architecture we need to be aware
...
% % processing element.
% Therefore, we annotate the task graph with the amount of SIMD
...
% % task graph across the available resources. 
...
% challenge to divide the tasks. Moreover, this causes the communication
...
% between the resources when mapping the task graph.
...
%~ David's Text
%~ In addition to allocating tasks to PEs with appropriate levels of SIMD
%~ parallelism, it is also important to divide the overall work of the
%~ task graph across the available PEs. However, there are two
...
%~ we partition the task graph across the clusters. At each level
...
%~ we divide the task graph among the clusters, until we reach the
%~ lowest level where each task is allocated to a specific processing
%~ element.
