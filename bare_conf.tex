...
While generating the resource graph, we might have to contract
edges(becaus we contract nodes). Contracting an edge $e=\{i,j\}$ means
to replace vertices $i$ and $j$ by a new vertex $k$ such that $R^k_0 =
R^i_0 + R^j_0$ and $R^k_1 = R^i_1 + R^j_1$. All edges of the form
$\{i,x\}$ and $\{j,x\}$ for $x \in V_r$ are replaced by $\{k,x\}$. If
both edges $\{i,x\}$ and $\{j,x\}$ exist, we form the new edge weight as
defined by $E^{\{w,x\}} = E^{\{i,x\}} + E^{\{j,x\}}$ More information
about the generation about the generation of the resource graphs is
provided later in Section~\ref{sec:gener-reso-graph}.
