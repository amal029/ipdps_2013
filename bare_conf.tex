...
\addtolength{\floatsep}{-9mm}
\addtolength{\textfloatsep}{-7mm}
\addtolength{\intextsep}{-3mm}
\addtolength{\dbltextfloatsep}{-7mm}
...
\title{Exploiting filter and data parallelism on heterogeneous
  architectures in a HPC environment}
...
Shoukat Ali\IEEEauthorrefmark{4} and
...
Email: avinmali@ie.ibm.com, drshoukatali@gmail.com}
}
...
\begin{abstract}
  ...
  heterogeneous HPC architectures. Given an HPC cluster with varying
  compute units, communication constraints and topology, our framework
  can be utilized for partitioning applications exhibiting filter and data
  ...
  % compute units needed and the topology that would be suited for a
  % given
  % application, thereby reducing costs and power requirements.
  The challenge lies in the fact that heterogeneous compute clusters
  consist of processing elements exhibiting different compute speeds,
  vector lengths, and communication bandwidths, which all need to be
  considered when partitioning the application and associated data. We
  tackle this problem using a staged graph partitioning framework. Our
  experiments show orders of magnitude speed up for
  applications. Furthermore our framework finishes within seconds even
  for architectures with 100's of processing elements, which makes our
  architecture suitable for exploring parallelism potential off line or
  on line.
...
\begin{figure*}[t!]
  ...
  \caption{Jacobi example and its stream-graph}
  \label{fig:1}
\end{figure*}
...
In the streaming model statements (also called filters/actors or
kernels) execute iteratively processing the incoming tokens of
data. Consider the Jacobi example and its filter graph in
Figure~\ref{fig:1}, the Jacobi kernel is used in fluid dynamics and heat
transfer problems. We consider every statement (marked $1$ to $4$) in
this example to be a kernel statement that can be run in a software
...
In a software pipelined model, the different iterations of the
statements are run in parallel, e.g., $1_0$ is the $1^{st}$ iteration of
statement $1$ in the Jacobi example, while $1_1$ is the second iteration
and so on and so forth. The latency of the application (termed makespan)
is the period, which iterates continuously (shown within the double
lined columns in Table~\ref{tab:3}). In such a model, the resource
allocation (rather than dependencies) determines the application
latency, especially without back-edges in the filter graph (as is the case
with our model and compiler). In Table~\ref{tab:3}, the resource
allocation on processing element \texttt{P1} determines the application
latency, because that is the maximum of the two allocation latencies.
...
$V_t$ is the set of all filters in the application graph and $E_t$
represents the communication between these filters. The system resources
are represented by a weighted undirected graph $G_r(V_r, E_r)$ where
$V_r$ represents a set of processing elements (PEs) which can have
different processing capabilities and $E_r$ represent the communication
links between these PEs with differing latencies and bandwidths. Each
vertex in the filter graph, $t_i \in V_t$ is referred to as a filter and
each vertex in the resource graph, $r_i \in V_r$ is referred to as a
processing element (PE). % We use $N_T$ to denote
% the total number of filters in the filter graph and $N_R$ to denote the
...
The communication edges are decorated with \mbox{$e^c, \forall E_t \in (V_t \times
V_t)$}, which denotes the data the filter requires for processing.
%~ $(t_i,t_j) \in E_t$
%~ between data-stores (statements without the constraints
%~ in Figure~\ref{fig:1}) and the execution statements
...
denotes a processing element (PE).
%~ It consists of a set of vertices $V_r =
%~ \{r_1, r_2, ... , r_n\}$ and a set of edges $E_r$.
Each resource node $r_i \in V_r$ has a number of computational capabilities,
represented by $R^{i}_{j}$ where $j=0...n_r$.
...
formulation for some filter $i$ being mapped onto some resource $j$, we
...
the total number of instructions to be performed by that stream-graph
node. Once we have this number we calculate latency of execution of this
stream-graph node $i$ on this resource $j$ by dividing it with the MIPS
value of the resource denoted by $R^j_0$. Calculation of the
communication latency requires dividing the number of bits
by the bandwidth of the shortest path.

Given the stream-graph and the resource-graph, let $\mathcal{M}$ be some
...
\begin{equation}
  \begin{array}{c}
    ...
    Lcomm^{\mathcal{M}}_s\\
    \\Lcomp^{\mathcal{M}}_s =
    \sum_{\forall i \in V_t} ((T^i_1/R^s_1\times T^i_0)/R^s_0)\\
    \\Lcomm^{\mathcal{M}}_s =
...
\begin{equation}
  \label{eq:2}
  L^{\mathcal{M}} = max ({L^{\mathcal{M}}_s}), \forall s \in V_r
\end{equation}
\end{scriptsize}
...
% quickly gauge the type of underlying architecture that is best suited
% to run a given application. Moreover,
