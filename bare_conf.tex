...
\title{Exploiting task and data parallelism on heterogeneous
...
\footnotetext[\value{footnote}]{Ellipses represent data stores. Rectangles
  represent task nodes. Rounded rectangle represents data parallel
...
\begin{figure*}[t!]
  ...
  \caption{Jacobi example and its task-graph}
  \label{fig:1}
\end{figure*}
...
data. Consider the Jacobi example and its task graph in
...
latency, especially without back-edges in the task graph (as is the case
...
vertex in the task graph, $t_i \in V_t$ is referred to as a task and
...
Our framework considers both task and data parallelism, which allows the
...
underlying architecture edges, which when combined with the task and
