...
\item A set $\bw = \{B_1, B_2, B_3, ... B_{|B|}\}$ where $B_i$ is a power of 2.
  Every CPU/GPU in this experiment has a bandwidth of $B_i$ in MB/s where $B_i$
...
\begin{figure*}[ht!]
  \centering
  \subfigure[\centering CPU intensive graphs
  ($\beta-{5,25,50}$) ]{
    \includegraphics[angle=0, scale=0.44]{./figures/cpu}
    \label{fig:cpu}
  }
  \subfigure[\centering GPU intensive graphs ($\beta-{50,75,95}$)]{
    \includegraphics[angle=0, scale=0.44]{./figures/gpu}
    \label{fig:gpu}
  }
  \subfigure[\centering Serial applications
  ($\alpha-{1,0.1}$ $\beta-{50}$)]{
    \includegraphics[angle=0, scale=0.44]{./figures/serial}
    \label{fig:serial}
  }
  \subfigure[\centering Task parallel applications
  ($\alpha-{10,1}$ $\beta-{50}$)]{
    \includegraphics[angle=0, scale=0.44]{./figures/parallel}
    \label{fig:parallel}
  }
  \subfigure[\centering Comm. intensive graphs
  ($\alpha-1$ $\beta-50$ $c-1$)]{
  ...
  \caption{\centering Experimental results based on random application
...
finally \re{fig:overall} shows the average application latency based on all
the input graphs. Overall it just took us XXX time over metis for the biggest
architecture of 4096 nodes to determine a partition.
...
archtiectures when compared with metis alone.
...
with high bandwidths are combined together at each level.
