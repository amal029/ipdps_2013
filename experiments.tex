...
\item Beta ($\beta$) - We use this parameter to decide if an actor in
  ...
  the second constraint larger than the first). What this essentially 
  means is that smaller values of $\beta$ creates filters which require
  lot more scalar requirement, whereas larger values would result in filters
...
We generated one graph per combination for a total of 7875 application graphs.
...
We ran over 100,000 experiments for using the random application graphs
and our HMPA heuristic and K-way partitioning..
The results comparing K-way partitioning using metis and HMAP framework
are shown in figure \ref{fig:exp-res}. In all the graphs the applications
graphs that matches a particular criteria are partitioned for a given architecture
and the latency corresponding latency is calculated. This is then averaged
for all graphs that belong to the group and these are plotted across the different
architectures.

The results are divided into five
...
biggest architecture of 4096 nodes to determine a good partition of the 
...
The graphs with values for $\beta - 5,25,50$ represents those with more scalar
requirements i.e. CPU intensive filters. Whereas the graphs with 
$\beta - 50,25,95$ are those with more vector requirements. On resource architecture
size of 64 nodes metis performs 1.2x faster than our heuristic but on all bigger
sizes we start to perform better and for the largest one of 4096 nodes we perform
over 3x faster than k-way partioning alone.

In the case of figures \ref{fig:serial} and \ref{fig:parallel} which represents
applications that belong to $\alpha-1,0.1  \beta-50$(serial) and $\alpha-10,1 \beta-50$
(parallel) respectively.
 are tall consisting of more filters that can only be
...
We also observed that metis performs better than our framework in the case of
